...
static void __inline__ fec_get_mac(struct net_device *ndev)
{
	...
	printk("lixw::MAC:%x,%x,%x,%x,%x,%x::addr:0x%x\n",macaddr[0],macaddr[1],macaddr[2],macaddr[3],macaddr[4],macaddr[5],fep->hwp);
...
static int fec_enet_mdio_read(struct mii_bus *bus, int mii_id, int regnum)
{
	...
	udelay(1000);	
	unsigned int preamle = 0xffffffff;
	unsigned short mdio = (FEC_MII_READ(mii_id, regnum) & 0xffff0000 ) >> 16;
	unsigned short data;
	for(i = 0, mark =1; i < 32; ++i)
        {
		//gpio_set_value(PHY_MDIO, preamle & mark);
		gpio_set_value(PHY_MDIO, 1);
		
		gpio_set_value(PHY_MDC, 0);
		gpio_set_value(PHY_MDC, 1);
		mark <<= 1;
        }
	for(i = 0, mark =0x8000; i < 16; ++i)
        {
		//gpio_set_value(PHY_MDIO, mdio & mark);
		if(mdio & mark)
		gpio_set_value(PHY_MDIO, 1);
		else
		gpio_set_value(PHY_MDIO, 0);
	...
	//udelay(1);
	gpio_direction_input(PHY_MDIO);
	udelay(1);
	for(i = 0; i < 16; ++i)
        {
		data = gpio_get_value(PHY_MDIO);
		gpio_set_value(PHY_MDC, 0);
		gpio_set_value(PHY_MDC, 1);
		value |=  data;
		if(i < 15)
	...
	//printk("lixw::mdio-read:0x%x,addr:0x%x,reg:%d,value:0x%x\n",mdio, mii_id,regnum,value);
	return value;
#endif
}
			   ...
			   u16 value)
{
	...
	udelay(1000);	
	unsigned int preamle = 0xffffffff;
	unsigned short mdio = (FEC_MII_WRITE(mii_id, regnum, value) & 0xffff0000 ) >> 16;
	unsigned short data;
	for(i = 0, mark =1; i < 32; ++i)
        {
		//gpio_set_value(PHY_MDIO, preamle & mark);
	...
	//printk("lixw::mdio-write:0x%x,addr:0x%x,reg:%d,value:0x%x\n",mdio, mii_id, regnum, value);
	return 0;
#endif
}
